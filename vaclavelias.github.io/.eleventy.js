...
module.exports = function (eleventyConfig) {
    ...
    eleventyConfig.addShortcode("video", function (url, imageExtension = "jpg") {
        return `<div class="ratio ratio-16x9 mb-2"><video autoplay controls loop preload="none" poster="${url.replace(".mp4", `.${imageExtension}`)}"><source src="${url}" type="video/mp4"></video></div>`;
    });

    eleventyConfig.addShortcode("video-fluid", function (url, imageExtension = "jpg") {
        return `<video class="mb-2 img-fluid" autoplay controls loop preload="none" poster="${url.replace(".mp4", `.${imageExtension}`)}"><source src="${url}" type="video/mp4"></video>`;
    });
